const App = {
    state: {
        ...
        wishlist: JSON.parse(localStorage.getItem('jambWishlist') || '[]'),
    ...
    renderQuestionCard(q, subjectKey, showWishlist = false) {
        const subj = this.subjects[subjectKey];
        const isBookmarked = this.state.bookmarks.includes(q.id);
        const isWishlisted = this.state.wishlist.includes(q.id);
        return `
            <div class="study-question-card" id="sq-${q.id}">
                ...
                <div class="q-actions">
                    ...
                    ${showWishlist ? `<button class="btn btn-sm ${isWishlisted ? 'btn-primary' : 'btn-outline'}" onclick="App.toggleWishlist('${q.id}')"><i class="fas fa-star"></i> ${isWishlisted ? 'Wishlisted' : 'Add to Wishlist'}</button>` : ''}
    ...
    renderBookmarks() {
        ...
        container.innerHTML = questions.map(q => this.renderQuestionCard(q, q.subjectKey, true)).join('');
    },
    ...
    toggleWishlist(qId) {
        const idx = this.state.wishlist.indexOf(qId);
        if (idx === -1) {
            this.state.wishlist.push(qId);
            this.showToast('Added to wishlist!', 'success');
        } else {
            this.state.wishlist.splice(idx, 1);
            this.showToast('Removed from wishlist.', 'info');
        }
        localStorage.setItem('jambWishlist', JSON.stringify(this.state.wishlist));
        this.renderWishlist();
        this.renderStudyQuestions();
        this.updateDashboard();
        this.updateProgress();
    ...
    renderWishlist() {
        ...
        meta.innerHTML = `<div class="stat-card" style="flex:1"><div class="stat-icon"><i class="fas fa-star"></i></div><div class="stat-info"><span class="stat-value">${this.state.wishlist.length}</span><span class="stat-label">Wishlisted Questions</span></div></div>`;
    ...
    navigateTo(page) {
        ...
        switch (page) {
            ...
            case 'wishlist': this.renderWishlist(); break;
